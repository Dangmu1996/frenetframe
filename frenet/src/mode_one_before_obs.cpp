...
private:
    ...
    vector<geometry_msgs::Point> obs_vec;
    ...
    void calcMiddleLine()
    {
        this->middle_line_[0]=(left_line_[0]+right_line_[0])/2;
        this->middle_line_[1]=(left_line_[1]+right_line_[1])/2;
        this->middle_line_[2]=(left_line_[2]+right_line_[2])/2;
        this->middle_line_[3]=(left_line_[3]+right_line_[3])/2;
    }
    ...
    vector<FrenetPath> calcFrenetPaths(float c_speed, float c_accel, float c_d, float c_d_d, float c_d_dd, float s0)
    {
        ...
        {
            for(float Ti = 0.1; Ti <= 3; Ti=Ti+0.1) // min time and max time, dt
            {
                FrenetPath fp;
                ...
                for(float td_d = -0.05; td_d<=0.05; td_d=td_d+0.01) //about sampling speed
                {
                    FrenetPath tfp = fp;
                    float Jp=0, Js=0;
    ...
    vector<FrenetPath> checkPaths(vector<FrenetPath> fplist)
    {
        ...
        for(auto &i: fplist)
        {
            ...
            for(auto &j: i.c)
            {
                if(abs(j)>5.0) //max_curvature(1.0m/s)
                {
                    flag=true;
                    break;
                }
    ...
    bool checkCollision(FrenetPath fp)
    {
        bool flag = false;
        float minimum_dis = 0.4;
